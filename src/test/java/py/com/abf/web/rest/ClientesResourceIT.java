...
class ClientesResourceIT {
    ...
    @Test
    @Transactional
    void getAllClientesByFacturasIsEqualToSomething() throws Exception {
        Facturas facturas;
        ...
        em.persist(facturas);
        em.flush();
        clientes.addFacturas(facturas);
        clientesRepository.saveAndFlush(clientes);
        Long facturasId = facturas.getId();

        // Get all the clientesList where facturas equals to facturasId
        defaultClientesShouldBeFound("facturasId.equals=" + facturasId);

        // Get all the clientesList where facturas equals to (facturasId + 1)
        defaultClientesShouldNotBeFound("facturasId.equals=" + (facturasId + 1));
    }

...
public interface FacturasRepository extends JpaRepository<Facturas, Long>, JpaSpecificationExecutor<Facturas> {
    ...
    @Query(
        value = "select distinct facturas from Facturas facturas left join fetch facturas.alumnos",
    ...
    @Query("select distinct facturas from Facturas facturas left join fetch facturas.alumnos")
    List<Facturas> findAllWithToOneRelationships();

    @Query("select facturas from Facturas facturas left join fetch facturas.alumnos where facturas.id =:id")
